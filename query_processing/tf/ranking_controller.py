...
from ..tf.ranking import match_and_rank, IndexDataLoader
import logging
...
async def match_and_rank_api(request: RankingRequest):
    ...
    """
    try:
        # Preload the index data
        index_loader.load(request.dataset_name)
        
        # Perform the ranking
        ranked_results = match_and_rank(
...
async def preload_dataset(dataset_name: str):
    ...
    """
    try:
        index_loader.load(dataset_name)
