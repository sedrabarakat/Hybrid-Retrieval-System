...
def get_qrels_path(dataset_name: str) -> str:
    """
    يرجع مسار ملف qrels حسب اسم الداتاسيت.
    عدّل المسارات هنا إذا أضفت داتاسيت جديد.
    """
    paths = {
...
def get_queries_from_db(dataset_name: str):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, query_text FROM queries WHERE dataset_name = %s limit 3",
        (dataset_name,)
    )
...
def run_evaluation(dataset_name: str):
    ...
    # نحول المفاتيح إلى string لأن بعض أنواع المفاتيح (مثل Measure) لا تُخزَّن مباشرة في JSON
