...
def build_and_view_clusters(dataset_name: str, n_clusters: int, top_terms=True):
    ...
    start_time_silhouette = time.time()
    try:
        # تم إزالة منطق أخذ العينات
        silhouette = silhouette_score(loaded_tfidf_matrix, clusters) # حساب على كل المستندات
        print(f"Silhouette Score (full dataset) for {dataset_name}: {silhouette:.4f}")
    except Exception as e:
        print(f"Could not calculate Silhouette Score: {e}. This might be due to extreme memory constraints or data characteristics. Consider re-enabling sampling for this metric if it fails.")
        silhouette = None
    end_time_silhouette = time.time()
    print(f"Silhouette Score calculation completed in {end_time_silhouette - start_time_silhouette:.2f} seconds.")
